...
public class PostService {
    ...
    public Long create(CreatePostRequest request) {
        ...
        // 🔥 Set publishedAt only if post is marked as published
        if (Boolean.TRUE.equals(entityToSave.getPublished())) {
    ...
    public void update(Long id, UpdatePostRequest request) throws NotFoundException{
        var postEntity = this.findEntityById(id);
        var updatedPostEntity = postMapper.updateRequestToEntity(request, postEntity);
